...
int main(int argc, char *argv[]) {
    ...
    if (args.show_version) {                        // If --version flag is set
        print_version();                            // Print version information
        return 0;                                   // Exit program
    }
    if (args.show_help) {     // If --help flag is set
    ...
    FileEntry *entries =
        malloc(MAX_ENTRIES * sizeof(FileEntry));  // Allocate memory for file entries
    if (entries == NULL) {                        // Check if memory allocation failed
    ...
    if (is_dev_directory(current_dir)) {  // Check if current directory is /dev
        handle_dev_directory(&entries, &num_entries,
                             MAX_ENTRIES);  // Handle /dev directory specially
    } else {
        ...
        while ((entry = readdir(dir)) != NULL &&
               num_entries < MAX_ENTRIES) {  // Read directory entries
            if (strcmp(entry->d_name, ".") == 0 ||
                strcmp(entry->d_name, "..") == 0) {  // Skip . and .. entries
                continue;
            }

            if (create_file_entry(&entries[num_entries], entry->d_name) !=
                0) {  // Create file entry
                fprintf(stderr, "Warning: Unable to get info for %s: %s\n", entry->d_name,
                        strerror(errno));  // Print warning if creation fails
    ...
    qsort(entries, num_entries, sizeof(FileEntry), compare_file_entries);  // Sort entries

    display_entries(entries, num_entries, term_width);  // Display sorted entries

    for (int i = 0; i < num_entries; i++) {  // Loop through all entries
        free_file_entry(&entries[i]);        // Free each entry
    }
